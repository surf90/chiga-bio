...
### 2026-09-25 — 詳細読み込み失敗時の応急処置欄の明示（[PR #62](https://github.com/surf90/chiga-bio/pull/62)）

不具合修正:

- オフライン等で `species/{id}.json` を取得できないとき、一覧用の軽量データでモーダルを開くため、危険生物でも応急処置の欄が黙って消えていた。`_partial` を付け、「詳細情報（応急処置など）を読み込めませんでした」を `.alert-box` で表示するようにした
- `sw.js` の `CACHE_VERSION` を v1.8.1 に更新

ドキュメント:

- README に「データの扱い」（外部読み込み先・危険生物の対処の位置づけ）とライセンスを追記し、CHANGELOG.md を追加

### 2026-09-02 — 表示・操作性の改善（ヘッダー・ボトムナビ・件数表示）

不具合修正:

- ヘッダー縮小時に説明文（`h1 small`）だけが消え、区切り線「|」が宙に浮いて残っていた表示崩れを修正。区切り線も併せて隠すようにした

操作性の改善:

- ボトムナビが画面幅に収まらないとき（390px 幅では 619px 必要で2項目が画面外）、スクロールバーを隠しているため続きがあると気づけなかった問題に対応。スクロールできる向きの端をフェードさせて手がかりを出すようにした
- フェードを帯全体に掛けると背景ごと透けて下のカードが見えてしまうため、背景・影を持つ外側の `.bottom-nav` と、横スクロールとフェードを担う内側の `.bottom-nav-scroller` に分離した
- 端の項目を選ぶと見切れたままになっていたため、選択項目をナビの可視域へ寄せるようにした。`scrollIntoView` はページ全体もスクロールし直前の `scrollToTop()` と競合するため、ナビの横スクロールだけを動かしている
- 絞り込み中（検索語あり、または場所フィルタが「すべて」以外）のときだけ、件数を目に見える形でも表示するようにした。全件表示のときは従来どおり読み上げ専用（`sr-only`）に戻る。`#result-count` は一覧の上へ移動

その他:

- `sw.js` の `CACHE_VERSION` を `v1.8.0` へ bump（HTML/CSS/JS を変更したため）

### 2026-09-02 — GitHub Pages 配信設定と画像キャッシュの改善

配信・デプロイ:

- Pages デプロイの `concurrency` を `cancel-in-progress: false` に変更（GitHub 公式スターターワークフローの推奨。進行中の本番デプロイが後続 push に中断されないようにした）
- PR とプッシュでビルドを検証する `ci.yml` を追加（`bio-data.json` のパースエラー・`id` 重複・生成物の欠落をデプロイ前に検出）
- `dist/404.html`（GitHub Pages のカスタム404）を生成。存在しないURLに着地しても一覧へ復帰でき、`noindex,follow` を付与
- `dist/.nojekyll` を生成し、Jekyll 変換を抑止

パフォーマンス:

- Service Worker が iNaturalist の画像を一切キャッシュできていなかった問題を修正。`<img>` 経由の取得は `type='opaque'`（`status=0`）で返るため従来の判定で弾かれていた。opaque を許可し、再訪時の画像表示を高速化（`CACHE_VERSION` を `v1.7.0` へ）
- 画像のキャッシュ探索を画像キャッシュに限定し、上限超過時の削除を一括処理へ整理
- `list.json` のプリロードから `crossorigin` を除去（`fetch()` の資格情報モードと食い違い、プリロードが使われず二重取得になっていた）
- モーダルの主画像に `fetchpriority="high"` を付与

その他:

- 配信サブパスを `<meta name="site-base">` としてビルド時に埋め込み、任意の深さで着地する404ページでもサイトルートを正しく解決できるようにした

### 2026-08-08 — 表示・操作の不具合修正とモダン化

不具合修正:

- カードを開いた状態のまま「戻る／進む」で別種へ切り替えると、退避していたスクロール位置とフォーカス元が上書きされ、閉じたときに一覧の先頭へ飛ぶ不具合を修正
- モーダルを閉じる際のフォーカス復帰が画面外のカードを画面内へ引き寄せ、復元したスクロール位置を上書きしていた問題を `focus({ preventScroll: true })` で修正
- モーダル表示中は `body` が `position: fixed` になり `scrollY` が 0 と評価されるため、ヘッダーが開いてしまい閉じたときに高さ差ぶんスクロール位置がずれる問題を修正
- SPA遷移で URL が `/species/{id}/` に変わるとアイコン・マニフェストの相対パスが再解決され、存在しない `/species/{id}/favicon.svg` などを取得して404になっていた問題を修正（読み込み時に絶対URLへ固定）
- 画像取得失敗時のプレースホルダー差し替えで `srcset` が残り、`src` より優先されて画像が壊れたままになる問題を修正
- トーストを連続表示すると先行のタイマーが後発を早く消してしまう問題を修正
- スワイプ開始判定を真偽フラグ化し、`clientY` が 0 のときにモーダルのスワイプが効かない問題を修正。`touchcancel` でも表示位置を戻すようにした
- 場所フィルタのみで0件になったときに「「」に一致する〜」と空の鉤括弧が出る問題を修正
- 閉じているモーダル内の要素がタブ移動・読み上げの対象になっていた問題を `inert` と `visibility` で修正
- 折りたたまれた参考文献内のリンクをフォーカストラップが掴み、フォーカスが失われる問題を修正
- CSP（`style-src 'self'`）に違反していた `noscript` のインライン `style` をクラスへ移動
- `index.html` のプリロード先が実際に取得する `list.json` と食い違っていた問題を修正（`scripts/build.py` の置換処理も併せて整理）

表示のモダン化:

- ライト／ダークで切り替わる `theme-color` と `color-scheme` を追加し、フォームやスクロールバーを配色へ追従
- ダークモードのヘッダー・選択中ナビ・トーストが明るいブランド色になり白文字のコントラストが不足していたため、濃い側のブランド色へ分離（`--brand-surface-from` / `--brand-surface-to` / `--on-brand`）
- デスクトップ幅（700px以上）では下端シートではなく中央のダイアログとして表示
- 閉じる・シェア・クリア・リセット各ボタンへ `:focus-visible` の輪郭を追加
- 背景と同色で枠が見えなかったフッターの注意書きをカード面＋ヘアラインへ変更
- WebKit 標準の検索クリア「✕」を非表示にし、独自クリアボタンとの二重表示を解消
- モーション控えめ設定時は smooth スクロールを行わないようにした
- `sw.js` の `CACHE_VERSION` を `v1.5.6`→`v1.6.0` へbump
